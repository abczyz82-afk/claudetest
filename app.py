...
with sb_tab3:
    ...
    # Check if trade_history exists in session_state before calculating
    if "trade_history" not in st.session_state:
        st.session_state.trade_history = []
        
    _open_cnt  = sum(1 for t in st.session_state.trade_history if t.get("status") == "OPEN")
    _closed_cnt= sum(1 for t in st.session_state.trade_history if t.get("status") == "CLOSED")
    _win_cnt   = sum(1 for t in st.session_state.trade_history if t.get("status") == "CLOSED" and t.get("pnl_points", 0) > 0)
    _wr        = (_win_cnt / _closed_cnt * 100) if _closed_cnt > 0 else 0
    _total_pnl = sum(t.get("pnl_points", 0) for t in st.session_state.trade_history if t.get("status") == "CLOSED")
